<div style="font-family: system-ui, -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; line-height: 1.6;">
  ...
  <p style="font-size: 0.98rem; max-width: 920px; margin: 0.2rem auto 0;">
    A Julia simulation and visualization project for a <strong>3-DOF, two-link manipulator</strong> with a <strong>fixed base</strong> in 3D space.
  ...
  <ul style="max-width: 920px; margin: 0.5rem auto 0.2rem; padding-left: 1.2rem;">
    ...
    <li><strong>Visualization:</strong> rendered with <strong>raylib</strong> via a Julia binding, including an overlay panel and a simple suction-tool “ball” pick-and-place animation.</li>
  </ul>

  <p align="center" style="font-size: 1rem; color: #666; margin: 0.35rem auto 0;">
    Environment: <strong>Julia</strong> • Rendering: <strong>raylib</strong> • IK: <strong>Closed-form</strong>
  </p>
...
- START position `(x, y, z)` and GOAL position `(x, y, z)` are edited in the overlay panel.
- Click **PAUSE** to edit points; click **PLAY** to apply and restart the pick-and-place loop.
- Constraint enforced by IK: **z must be ≥ 0**
- Reachability enforced by IK: `|p|` must be within the arm’s reachable shell.

A fixed EE “HOME” point is used:
...
- The runtime uses the default elbow branch (elbow-down).
...
They are displayed in the overlay panel and can be used as a foundation for:
...
- Julia **1.10+**
- raylib Julia binding (downloaded automatically by Julia’s package manager)
...
- Overlay panel:
  - **PAUSE** to stop time
  - edit START/GOAL fields
  - **PLAY** to apply new points and restart the pick-and-place loop
...
Defines a Julia project environment:

- pins dependencies (raylib binding)
- allows reproducible `Pkg.instantiate()` setup
...
- initializes window, camera, and UI font (optional)
- implements a pick-and-place finite-state machine:
...
- renders:
  ...
  - overlay panel (status, parameters, pause/play, input fields)

---

### `src/robot/RobotArm.jl`
...
- editable numeric input fields for START and GOAL
- reachability indicators and runtime error messages
- displays:
  - link lengths, masses, inertias
  - workspace bounds
  - applied start/goal norms and coordinates

---

### `src/render/DrawUtils.jl`
...
<a href="https://www.youtube.com/watch?v=DDTzISencmE" target="_blank">
  <img
    src="https://i.ytimg.com/vi/DDTzISencmE/maxresdefault.jpg"
    alt="3D simulation of an RRR manipulator with Julia"
...
### Julia can’t instantiate dependencies

- Ensure your Julia version is **1.10+**
- Try clearing the environment and re-instantiating:
  ```bash
  rm -f Manifest.toml
  julia --project=. -e "using Pkg; Pkg.instantiate()"
  ```

### Window opens but nothing moves

- If the overlay is in **PAUSE**, click **PLAY** to start the loop.
...
- The project will fall back to a system font (or raylib default).
- If you want the exact UI font, place `Inter-Regular.ttf` at:
